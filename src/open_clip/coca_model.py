...
class CoCa(nn.Module):
    def __init__(
        ...
        self.text = _build_input_dependent_text_tower(embed_dim, text_cfg, quick_gelu, cast_dtype, multimodal=False)
    ...
    def _build_cls_mask(self, text, cast_dtype):
        ...
        cls_mask = F.pad(cls_mask, (1, 0, cls_mask.shape[1], 0), value=True)
        additive_mask = torch.empty(*cls_mask.shape, dtype=cast_dtype)
    ...
    def encode_text(self, text, normalize=True, return_tokens=False):
        ...
        x = self.text.transformer(x, attn_mask=self.attn_mask)
        x = x.permute(1, 0, 2)  # LND -> NLD

        # x.shape = [batch_size, n_ctx, transformer.width]
        # take features from the eot embedding (eot_token is the highest number in each sequence)
        x = x[torch.arange(x.shape[0]), :] @ self.text.text_projection

        cls_emb = x[torch.arange(x.shape[0]), -1]
        token_emb = x[torch.arange(x.shape[0]), :-1]

        cls_emb = self.text.ln_final(cls_emb)
