...
def train_one_epoch(model, data, loss, epoch, optimizer, scaler, scheduler, dist_model, args, tb_writer=None, dino_schedulers=None):
    ...
    for i, batch in enumerate(dataloader):
        ...
        if args.dino_config_file is None:
            ...
            _, texts, dino_images = batch
            # images = images.to(device=device, dtype=input_dtype, non_blocking=True)
            dino_images = {i:j.to(device=device, dtype=input_dtype, non_blocking=True) if isinstance(j, torch.Tensor) else j for i,j in dino_images.items()}
            texts = texts.to(device=device, non_blocking=True)
            images = dino_images
        ...
        with torch.no_grad():
            if unwrap_model(model).logit_scale is not None:
        ...
        if is_master(args) and (i_accum % args.log_every_n_steps == 0 or batch_count == num_batches_per_epoch):
            ...
            if args.dino_config_file is not None:
                batch_size = len(images["collated_global_crops"]) // 2
